...
static ros::Publisher lanelet_marker_pub;
static ros::Publisher lanelet_marker_array_pub;
static ros::Publisher lanelet_polygon_array_pub;
static ros::Publisher road_lanelet_polygon_array_pub;
static ros::Publisher road_lanelet_marker_array_pub;
static ros::Publisher stop_line_marker_array_pub;
static ros::Publisher traffic_light_marker_array_pub;
...
double lanelet_width(lanelet::Lanelet ll) {
  lanelet::LineString3d lb = ll.leftBound();
  lanelet::LineString3d rb = ll.rightBound();
  
  return ((lanelet::geometry::distance(lanelet::utils::to2D(lb.front()), lanelet::utils::to2D(rb.front())) +
	   lanelet::geometry::distance(lanelet::utils::to2D(lb.back()), lanelet::utils::to2D(rb.back())))/2.0);
...
int findClosestCrosswalk(lanelet::Lanelets crosswalks, const int closest_waypoint, const autoware_msgs::Lane &lane_msg, const int search_distance,
			 lanelet::Lanelets & closest_crosswalks, bool multiple_crosswalk_detection)
{
  int wp_near_crosswalk = -1;

  double find_distance = 2.0; //meter
  double ignore_distance = 20.0; // meter
  float cw_lane[4] = {1, 0, 0, 0.3}; 
  //  std::cerr << "in findclosest crosswalk\n\n\n";
  // find near crosswalk
  int cw_count = 0;


  
  for (int wpi = closest_waypoint; wpi < closest_waypoint + search_distance &&wpi < (int) lane_msg.waypoints.size(); wpi++) {
    // std::cerr << " wpi = " << wpi << "\n";
    ...
    //    lanelet::BoundingBox2d bbox(lanelet::BasicPoint2d(waypoint.x-ignore_distance, waypoint.y-ignore_distance),lanelet::BasicPoint2d(waypoint.x+ignore_distance, waypoint.y+ignore_distance));
  
    //    lanelet::Lanelets ll_inregion = lanelet_map->laneletLayer.search(bbox);
    // lanelet_utils::Visualization::publishLaneletsAsMarkers(ll_inregion, lanelet_marker_pub);

    for (auto li = crosswalks.begin(); li != crosswalks.end(); li++) {
	...
	//	std::cerr <<" extracted attribute: " << attr.value() << "\n";
	if (attr.value() == lanelet::AttributeValueString::Crosswalk) {
	  // std::cerr << "Found a CROSSWALK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << "\n";
	  // lanelet_utils::Visualization::publishLaneletAsMarkers(ll, lanelet_marker_pub, cw_lane, cw_lane, "crosswalk", cw_count);

	  double d = lanelet::geometry::distance2d(ll, wp2d); 
	  if (d < find_distance){
	    
	    //std::cerr << "distance of waypoint to crosswalk lane = " << d<< "\n";
  ...
  //  std::cerr << "return from findclaosest crosswalk\n";
...
EControl crossWalkDetection(const pcl::PointCloud<pcl::PointXYZ>& points, lanelet::Lanelets closest_crosswalks, //const CrossWalk& crosswalk,
                            const geometry_msgs::PoseStamped& localizer_pose, const int points_threshold,
                            ObstaclePoints* obstacle_points)
{

  // search radius is half the width of the first crosswalk detected

  double search_radius = lanelet_width(closest_crosswalks.front())/2.0;
  ...
  // for each crosswalk (old - detectionCrossWalkIDs: new closest_crosswalks)
  // for each detection points defined for cross walk
  // for each point in detected object point cloud
  ...
  for (auto lli = closest_crosswalks.begin(); lli != closest_crosswalks.end(); lli++) {
	...
	double distance = lanelet::geometry::distance2d(ll, p2d);
...
int detectStopObstacle(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint, int detection_waypoint,
                       const autoware_msgs::Lane& lane, lanelet::Lanelets closest_crosswalks, //const CrossWalk& crosswalk,
		       double stop_range,
  ...
  *obstacle_type = EObstacleType::NONE;
  ...
  for (int i = closest_waypoint; i < closest_waypoint + STOP_SEARCH_DISTANCE; i++)
  {
    ...
    if (i == detection_waypoint)
    {
      ...
      if (crossWalkDetection(points, closest_crosswalks, localizer_pose, points_threshold, obstacle_points) == EControl::STOP)
      {
        stop_obstacle_waypoint = i;
...
//-------------------------------------------------------------------------
//
//  same as velocity_set.cpp - no reference to vector maps or croswalk
//
//-------------------------------------------------------------------------
...
EControl pointsDetection(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint, const int detection_waypoint,
                         const autoware_msgs::Lane& lane, lanelet::Lanelets closest_crosswalks, //const CrossWalk& crosswalk,
			 const VelocitySetInfo& vs_info,
                         int* obstacle_waypoint, ObstaclePoints* obstacle_points)
{
  ...
  int stop_obstacle_waypoint =0;
  detectStopObstacle(points, closest_waypoint, detection_waypoint, lane, closest_crosswalks,//crosswalk,
			 vs_info.getStopRange(),
...
void displayDetectionRange(const autoware_msgs::Lane& lane, lanelet::Lanelets closest_crosswalks, //const CrossWalk& crosswalk,
			   const int closest_waypoint, const int detection_waypoint, 
                           ...
                           const double deceleration_range, const ros::Publisher& detection_range_pub)
{
  ...
  //int crosswalk_id = crosswalk.getDetectionCrossWalkID();
  if (detection_waypoint > 0)
    scale = lanelet_width(closest_crosswalks.front());
  ...
  // how does detection points relate to lanelet crosswalk? just draw extent of lanelet?
  /*
  if (crosswalk_id > 0)
  {
...
EControl obstacleDetection(int closest_waypoint, int detection_waypoint, const autoware_msgs::Lane& lane, lanelet::Lanelets closest_crosswalks, // const CrossWalk& crosswalk,
                           ...
                           const ros::Publisher& obstacle_pub, int* obstacle_waypoint)
{
  ...
  EControl detection_result = pointsDetection(vs_info.getPoints(), closest_waypoint, detection_waypoint, lane, closest_crosswalks, //crosswalk,
					      vs_info,
                                              obstacle_waypoint, &obstacle_points);


  
  displayDetectionRange(lane, closest_crosswalks, closest_waypoint, detection_waypoint, detection_result, *obstacle_waypoint, vs_info.getStopRange(),
...
lanelet::Lanelets get_all_crosswalks(lanelet::LaneletMapPtr ll_map)
{
  lanelet::Lanelets cross_walks;
...
lanelet::Lanelets get_all_roads(lanelet::LaneletMapPtr ll_map)
{
  lanelet::Lanelets roads;
...
int main (int argc, char **argv)
{
  ...
  ros::NodeHandle rosnode, private_rosnode;
  ...
  ros::Subscriber waypoints_sub = rosnode.subscribe("safety_waypoints", 1, &VelocitySetPath::waypointsCallback, &vs_path);
  ...
  ros::Subscriber localizer_sub = rosnode.subscribe("localizer_pose", 1, &VelocitySetInfo::localizerPoseCallback, &vs_info);
  ros::Subscriber control_pose_sub = rosnode.subscribe("current_pose", 1, &VelocitySetInfo::controlPoseCallback, &vs_info);
  ros::Subscriber detectionresult_sub = rosnode.subscribe("/state/stopline_wpidx", 1, &VelocitySetInfo::detectionCallback, &vs_info);
  ...
  */
  ...
  lanelet_marker_pub = rosnode.advertise<visualization_msgs::Marker>("ll_inregion", 1000);
  lanelet_marker_array_pub = rosnode.advertise<visualization_msgs::MarkerArray>("crosswalk_lanelets", 1000);


  stop_line_marker_array_pub = rosnode.advertise<visualization_msgs::MarkerArray>("stop_lines", 1000);
  traffic_light_marker_array_pub = rosnode.advertise<visualization_msgs::MarkerArray>("traffic_lights", 1000);
  

  lanelet_polygon_array_pub = rosnode.advertise<jsk_recognition_msgs::PolygonArray>("crosswalk_lanelets_poly", 1000);
  road_lanelet_polygon_array_pub = rosnode.advertise<jsk_recognition_msgs::PolygonArray>("road_lanelets_poly", 1000);
  road_lanelet_marker_array_pub = rosnode.advertise<visualization_msgs::MarkerArray>("road_lanelets", 1000);
  

  
  ros::Publisher final_waypoints_pub;
  if(enablePlannerDynamicSwitch){
	  final_waypoints_pub = rosnode.advertise<autoware_msgs::Lane>("astar/final_waypoints", 1, true);
  }else{
	  final_waypoints_pub = rosnode.advertise<autoware_msgs::Lane>("final_waypoints", 1, true);
  }

  // get individual sets of primitive out of lanelets map for visualisation
  
  //lanelet::Lanelets crosswalk_lanelets = get_all_crosswalks(lanelet_map);
  ...
  // lanelet::Lanelets road_lanelets = get_all_roads(lanelet_map);
  std::vector<lanelet::ConstLineString3d> stop_lines = lanelet_utils::Query::stopLinesLanelets(road_lanelets);
  std::vector<lanelet::TrafficLight::Ptr> tl_reg_elems = lanelet_utils::Query::trafficLights(road_lanelets);
  

  std::cerr << "number of crosswalks =" <<  crosswalk_lanelets.size() << "\n";
  std::cerr << "number of roads =" <<  road_lanelets.size() << "\n";
  std::cerr << "number of stop lines =" << stop_lines.size();
  std::cerr << "number of traffic lights =" << tl_reg_elems.size();
  
  ros::spinOnce();
  //  lanelet_utils::Visualization::publishLaneletsAsMarkers(crosswalk_lanelets, lanelet_marker_pub);
  // lanelet_utils::Visualization::publishLaneletsAsMarkerArray(crosswalk_lanelets, lanelet_marker_array_pub);
  ros::Rate loop_rate(LOOP_RATE);


  float cw_ll_color_params[4] = { 1.0, 0.0, 0.0, 0.3};
  ...
  while (ros::ok())
  {
    ...
    //    lanelet_utils::Visualization::publishLaneletsAsMarkers(crosswalk_lanelets, lanelet_marker_pub);
    lanelet_utils::Visualization::publishLaneletsAsPolygonArray(crosswalk_lanelets, lanelet_polygon_array_pub);

    lanelet_utils::Visualization::publishLaneletsAsPolygonArray(road_lanelets, road_lanelet_polygon_array_pub);
    lanelet_utils::Visualization::publishLaneletsAsMarkerArray(road_lanelets, road_lanelet_marker_array_pub, road_ll_color_params);
  
    lanelet_utils::Visualization::publishLineStringsAsMarkerArray(stop_lines,stop_line_marker_array_pub, "stop_lines");
    lanelet_utils::Visualization::publishTrafficLightsAsMarkerArray(tl_reg_elems, traffic_light_marker_array_pub);
    ...
    if (use_crosswalk_detection){
      ...
      lanelet::Lanelets closest_crosswalks;
      int detection_waypoint = findClosestCrosswalk(crosswalk_lanelets, closest_waypoint, vs_path.getPrevWaypoints(), STOP_SEARCH_DISTANCE,
						    closest_crosswalks,enable_multiple_crosswalk_detection);
      
      
      int obstacle_waypoint = -1;
      EControl detection_result = obstacleDetection(closest_waypoint, detection_waypoint, vs_path.getPrevWaypoints(), closest_crosswalks,
						    vs_info,
